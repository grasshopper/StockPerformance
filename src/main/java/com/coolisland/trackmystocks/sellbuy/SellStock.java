...
public class SellStock {
		...
		try {
			...
			if (accounts != null && accounts.size() > 0) {
				...
				while (choice == RETRY_CHOICE) {
					System.out.println("Choose the account from which to sell a stock from the list below:");
					printAccountList(accounts);

					choice = InputUtils.getChoice(accountMap);
				}
	...
	private int chooseStock(int accountId) throws SQLException {
		...
		try {
			stockTickers = stockTickerDao.getOwnedStockTickersForAccount(accountId);

			if (stockTickers != null && stockTickers.size() > 0) {
				HashMap<Integer, String> stockTickerMap = createMapFromTickers(stockTickers);

				while (choice == RETRY_CHOICE) {
					System.out.println("Choose the stock to sell from the list below:");
					printStockTickerList(stockTickers);

//					choice = getStockTickerChoice(stockTickerMap);
					choice = InputUtils.getChoice(stockTickerMap);
					
					if (CANCEL_CHOICE == choice) {
						break;
					}
				}
...
//	}
	...
	private void sellStock(int accountId, int stockId) {
		StockDao stockTickerDao;

		try {
			stockTickerDao = new StockDao();

			stockTickerDao.udateOwnership(accountId, stockId, StockDao.Ownership.SELL);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
